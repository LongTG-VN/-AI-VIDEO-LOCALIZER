...
def to_ass(
    ...
    is_shortform = (
        options.visual_edit is not None
        and getattr(options.visual_edit, "preset", "default") == "shortform_reference"
    )

    font_name = options.font_name or "Arial"
    font_size = 23 if is_shortform else (options.font_size or 26)
    margin_v = 36 if is_shortform else (options.margin_v or 38)
    outline_w = 2.8 if is_shortform else (options.outline_width or 2.2)
    shadow_d = 1.0 if is_shortform else (options.shadow_depth or 0.8)
...
Style: Default,{font_name},{font_size},{font_color},&H000000FF,{outline_color},&H80000000,{bold_val},0,0,0,100,100,0,0,1,{outline_w},{shadow_d},2,20,20,{margin_v},1
