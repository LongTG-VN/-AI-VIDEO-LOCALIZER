...
class HardSubCleaner:
    """Removes burned-in hard subtitles with tight masks and optional temporal reconstruction."""
    ...
    @staticmethod
    def _component_rows(components: list[tuple[int, int, int, int, int]]) -> list[list[tuple[int, int, int, int, int]]]:
        if not components:
            return []
    ...
    def extract_text_mask(self, sub_roi: np.ndarray) -> np.ndarray | None:
        """Extract a conservative subtitle mask without swallowing bright background regions."""
        if sub_roi.size == 0:
            return None
        ...
        if not components:
            self._metrics["mask_rejected_no_components"] = int(self._metrics.get("mask_rejected_no_components", 0)) + 1
            return None
        ...
        for row in rows:
            ...
            if horizontal_span < max(10.0, median_h * 1.1):
                continue
            center_bias = 1.0 - min(1.0, abs(((x_min + x_max) / 2) - roi_w / 2) / max(1.0, roi_w / 2))
            score = len(row) * 2.0 + horizontal_span / max(8.0, median_h) + center_bias
            scored_rows.append((score, row))

        if not scored_rows:
            self._metrics["mask_rejected_no_text_row"] = int(self._metrics.get("mask_rejected_no_text_row", 0)) + 1
            return None
        ...
        if coverage <= 0.0 or coverage > self.max_mask_coverage:
            self._metrics["mask_rejected_coverage"] = int(self._metrics.get("mask_rejected_coverage", 0)) + 1
            return None
    ...
    def build_active_intervals(
        ...
        pad_seconds: float = 0.015,
    ) -> list[tuple[float, float]]:
        """Prefer OCR-backed cue timings so speech-only cues never trigger background destruction."""
        if not cues:
            return []
        ...
        valid = [
            cue
            for cue in cues
            if len((cue.source_text or "").strip()) > 1 and (cue.source_text or "").strip() not in NOISE_FILTER
        ]
        ocr_backed = [cue for cue in valid if cue.ocr_confidence is not None]
        selected = ocr_backed or valid
        self._metrics["timing_used_ocr_cues"] = len(ocr_backed)
        self._metrics["timing_used_fallback_cues"] = 0 if ocr_backed else len(valid)

        intervals = [
            (max(0.0, float(cue.start) - pad_seconds), float(cue.end) + pad_seconds)
            for cue in selected
        ]
    ...
    def _fast_inpaint_roi(self, roi: np.ndarray, mask: np.ndarray) -> np.ndarray:
        return cv2.inpaint(roi, mask, inpaintRadius=self.inpaint_radius, flags=cv2.INPAINT_TELEA)
    ...
    def _align_temporal_candidate(
        ...
        guard = cv2.dilate(mask, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5)), iterations=1)
        ...
        if initial_score > self.scene_cut_threshold * 2.0:
            self._metrics["temporal_scene_rejects"] = int(self._metrics.get("temporal_scene_rejects", 0)) + 1
            return None, None
        ...
        try:
            ...
            self._metrics["temporal_alignment_failures"] = int(self._metrics.get("temporal_alignment_failures", 0)) + 1
            return None, None

        aligned_gray = cv2.cvtColor(aligned, cv2.COLOR_BGR2GRAY)
        score = float(np.mean(cv2.absdiff(current_gray, aligned_gray)[valid > 0]))
    ...
    def _quality_inpaint_roi(
        ...
        fast = self._fast_inpaint_roi(roi, mask)
        best: np.ndarray | None = None
        best_score: float | None = None

        for donor in donors:
            aligned, score = self._align_temporal_candidate(roi, donor, mask)
            if aligned is None or score is None:
                continue
        ...
        if best is None or best_score is None:
            return fast, False, None

        confidence = max(0.0, min(1.0, 1.0 - best_score / self.scene_cut_threshold))
        temporal_weight = 0.55 + 0.30 * confidence
        hybrid = cv2.addWeighted(best, temporal_weight, fast, 1.0 - temporal_weight, 0.0)

        alpha = cv2.GaussianBlur(mask.astype(np.float32) / 255.0, (0, 0), sigmaX=0.75, sigmaY=0.75)
        alpha = np.clip(alpha * 1.15, 0.0, 1.0)[..., None]
        result = np.clip(roi.astype(np.float32) * (1.0 - alpha) + hybrid.astype(np.float32) * alpha, 0, 255).astype(np.uint8)
        return result, True, best_score
    ...
    def clean_frame(
        ...
        if mode in {"quality", "auto"} and temporal_donors:
            donor_rois = [donor[y1:y2, x1:x2] for donor in temporal_donors if donor.shape == frame.shape]
            cleaned_roi, used_temporal, temporal_score = self._quality_inpaint_roi(sub_roi, mask_roi, donor_rois)
            if used_temporal:
                self._metrics["temporal_frames"] = int(self._metrics.get("temporal_frames", 0)) + 1
                if temporal_score is not None:
                    self._metrics["temporal_score_sum"] = float(self._metrics.get("temporal_score_sum", 0.0)) + temporal_score
            else:
                self._metrics["fallback_inpaint_frames"] = int(self._metrics.get("fallback_inpaint_frames", 0)) + 1
        else:
    ...
    def clean_video(
        ...
        """Clean video using OCR-gated intervals, conservative masks, and temporal donors in quality mode."""
            ...
            while cap.isOpened():
                ...
                if donor_cap is not None:
                    if loaded_donor_interval != interval_idx:
                        ...
                        current_donors = [item for item in [prev_donor, next_donor] if item is not None]
        ...
        metrics = {
            ...
            "ocr_timed_cues": int(self._metrics.get("timing_used_ocr_cues", 0)),
            "fallback_timed_cues": int(self._metrics.get("timing_used_fallback_cues", 0)),
        ...
        logger.info("HardSub Quality V2 completed: %s", metrics)
        return metrics
